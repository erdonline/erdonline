...
- **改法**：
  ...
  2. `frontend/scripts/gen-seo-static.mjs`：
     - 把 SPA 壳写入 `dist/app`（无扩展名文件，避免 CF Pages 200 重写到目录时 308）。
  ...
  4. `frontend/public/_headers`：设置 `/app` 的 `Content-Type: text/html; charset=utf-8`；`favicon.ico` 改为 `immutable` 长缓存。
