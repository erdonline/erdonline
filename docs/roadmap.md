...
- **全站布局重设计总纲**：[ui-layout-redesign.md](./ui-layout-redesign.md)（2026-08-02 v2 重估：能力暴露优先于表现层；分波 W1 设计器壳 ✅ → **W2 能力暴露+空壳清除**（切片 1 ✅ 分享吊销；切片 2 ✅ Home 死码/实验页源文件物理删除 + charts/chatui 依赖移除；待：设计器 chrome 收尾）→ **W3 版本域收口**（version 摘 ProList + 跨版本 diff 导出 + 审批入口）→ W4 项目列表/数据源平移 → W5 登录/分享/404 + Pro 依赖一次性移除；能力对照见 [product-capability-map.md](./product-capability-map.md)）
