...
**下一阶段战略（服务北极星，不推翻愿景）**：优先 **P2b 全站控件闭环**（矩阵见 [control-matrix.md](./control-matrix.md)）；P4 Issue 草稿待投放；正式仓就绪后 `REPO=… ./scripts/seed-good-first-issues.sh`；AI / i18n 后置。
...
| **P2b：全站控件闭环** | 可点即可达结果；死入口修或删 | 控件矩阵 [control-matrix.md](./control-matrix.md)；~~W0–W6~~✅；~~ERD 导入~~✅；~~对比版本~~✅；~~逆向解析~~✅；~~数据源编辑/删除~~✅；~~同步状态钮~~✅；~~顶栏自动保存状态~~✅；~~命令面板/快捷键~~✅；~~默认字段~~✅；~~工单有数据拒绝/复批~~✅；~~数据域/查询侧栏裁剪~~✅；~~账户基本资料 toast~~✅；~~Home 数据查询主导航裁剪~~✅；~~settings 其它页签~~✅；~~群组基本设置 toast~~✅（残留：权限组 / 返回列表 / 打开模型 / 404） | ✅ 2026-08-02 |
