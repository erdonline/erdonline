...
- 逆向保真🚧：复合 FK `fields[]`（ADR-0011 解封条件）、~~PG 表/列注释 → chnname~~✅（字典 `obj_description`/`col_description`）、~~SQL Server 表/列注释 → chnname~~✅（`MS_Description`）、~~Oracle 表/列注释 → chnname~~✅（`ALL_TAB_COMMENTS`/`ALL_COL_COMMENTS`）、~~列默认值 `COLUMN_DEF` → `defaultValue`~~✅（JDBC 通用）、索引已字典化、~~MySQL 触发器 → `triggers[]`~~✅（`INFORMATION_SCHEMA.TRIGGERS`）、~~PG 触发器 → `triggers[]`~~✅（`information_schema.triggers`）、~~SQL Server 触发器 → `triggers[]`~~✅（`sys.triggers`）；Oracle 触发器另切片
