...
- 导入/导出互通：DBML / dbdiagram 格式互转，降低迁移成本；插件机制后置 — ✅（2026-08-02：导入+导出 Table/fields/FK/note↔chnname + Indexes↔`indexs` + `default`↔`defaultValue` 闭环；enum/表达式索引另切片；**trigger 文档延期**：`@dbml/core` 无块、`Note` 禁塞）
