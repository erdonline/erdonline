...
**下一阶段战略（服务北极星，不推翻愿景）**：P2 / P3a / P3b 雏形已齐；打磨协作冲突与逆向 FK；存量 eslint warn 与文档站（P4）后置；AI / i18n 仍📋。
...
| P3：功能深度 | 比竞品强 | ~~版本 diff 可视化~~✅（第 3 轮）；~~协作 presence+光标+增量 sync（ADR-0009）~~✅；~~远端同步冲突提示~~✅；~~只读分享链接~~✅（ADR-0007）；~~反向解析 Dialect SPI + schema API + FK~~✅（ADR-0006；~~MySQL 字典 FK+复合列序~~✅；PG/Oracle/SQLServer 字典 FK 与 `fields[]` 待续）；AI📋；i18n📋 | 🚧 |
