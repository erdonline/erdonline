...
| **P2b：全站控件闭环** | 可点即可达结果；死入口修或删 | 控件矩阵 [control-matrix.md](./control-matrix.md)；~~W0–W6~~✅；~~ERD 导入~~✅；~~对比版本~~✅；~~逆向解析~~✅；~~数据源编辑/删除~~✅；~~同步状态钮~~✅；~~顶栏自动保存状态~~✅；~~命令面板/快捷键~~✅；~~默认字段~~✅；~~工单有数据拒绝/复批~~✅；~~数据域/查询侧栏裁剪~~✅；~~账户基本资料 toast~~✅；~~Home 数据查询主导航裁剪~~✅（残留：settings 其它页签 / 群组设置等长尾） | ✅ 2026-08-02 |
