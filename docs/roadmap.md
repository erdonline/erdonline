...
- 数据字典 / 治理：字段级文档、~~枚举域~~✅（`/setting/dataType` kind=enum + `values[]` 可编）、跨表复用（承接 📋 dataDomain 实验页的定位重估）
