...
| P3：功能深度 | 比竞品强 | ~~版本 diff 可视化~~✅（第 3 轮）；~~协作 presence+光标+增量 sync（ADR-0009）~~✅；~~远端同步冲突提示~~✅；~~只读分享链接~~✅（ADR-0007）；~~反向解析 Dialect SPI + schema API + FK~~✅（ADR-0006；~~MySQL/PG/SQLServer 字典 FK~~✅；Oracle 字典 FK 与 `fields[]` 待续）；AI📋；i18n📋 | 🚧 |
