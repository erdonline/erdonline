...
| P3：功能深度 | 比竞品强 | ~~版本 diff 可视化~~✅（第 3 轮）；协作光标；~~只读分享链接~~✅（ADR-0007；API+脱敏+`/s/:token` 表清单+只读关系图；协作光标/进一步脱敏另切）；~~反向解析 Dialect SPI + schema API + FK 关联~~✅（ADR-0006；字典级 FK SQL / 复合键体验待打磨）；AI 辅助建模；i18n（英文优先） | 🚧 |
