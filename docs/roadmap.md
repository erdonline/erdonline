...
| **P2b：全站控件闭环** | 可点即可达结果；死入口修或删 | 控件矩阵 [control-matrix.md](./control-matrix.md)；~~W0 布局壳~~✅；~~W1 获客会话~~✅；W2 项目面🚧；W3 设计器核心🚧（删边持久化/undo✅）；W4 版本时光机🚧（重命名/删除✅）；W5 导入导出+数据源🚧（PdMan/HTML/ERD/DDL 下载/测试连接✅）；W6 外围裁剪🚧（死菜单/占位页/假 affordance 已删） | 🚧 |
