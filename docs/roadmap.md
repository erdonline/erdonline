...
| **P2b：全站控件闭环** | 可点即可达结果；死入口修或删 | 控件矩阵 [control-matrix.md](./control-matrix.md)；~~W0–W6~~✅；~~ERD 导入~~✅；~~对比版本~~✅；~~逆向解析~~✅；~~数据源编辑/删除~~✅；~~同步状态钮~~✅；~~顶栏自动保存状态~~✅；~~命令面板/快捷键~~✅；~~默认字段~~✅（残留：工单有数据链路、查询等长尾） | ✅ 2026-08-02 |
