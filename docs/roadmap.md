...
- CRUD 壳维持 antd（ADR-0005），设计域沉淀自研视觉系统（节点/工具条/命令面板已成体系）；[ADR-0016](./adr/0016-experience-first-shareable-diagram.md) 美图主线：品牌 token ✅ → 导入/逆向 dagre 分层布局 ✅ → 节点密度/PK·FK 徽章/箭头边 ✅ → 边路由（同表对多 FK 肘距分流 + 自定义 erdSmooth）✅ → 示例/默认布局密度（手排收紧 + dagre 64/120 + Frame padding 32）✅ → 边障碍避让（centerX / bypassY 绕中间表）✅ → **正交 edge bundling（同 midX 通道干道分流）✅** → 多弯 A* 续推
