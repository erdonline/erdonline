...
- [x] [B 层探测五态] 版本页进页 `schema-probe-status` 为 UNKNOWN/尚未探测；点「探测实库」loading → synced/ahead/behind/diverged/unknown（含 permission/unreachable copy）；进页不自动请求 `/connector/schema/probe` ✅ `schema-probe.spec.ts`
