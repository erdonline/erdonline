...
- **库内表**：系统/认证（用户、角色、菜单、OAuth、QRTZ…）与建模元数据（项目、版本、分享、`data_sources`、`catalog_*`…）共存。
- **双 SqlSessionFactory（过渡）**：`MartinDataSourceConfig` / `ErdDataSourceConfig` 仍按 mapper 包路由，但 JDBC 均指向同一 `MYSQLDATABASE`（默认 `erd`）。
