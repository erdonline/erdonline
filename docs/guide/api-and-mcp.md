---
title: 用一个 MCP URL 把 Agent 接到 ERD Online
description: 铸造 PAT 后在同一弹层获得已填 Streamable HTTP 配置；Cursor 等客户端读写同一份 projectJSON，端点是 https://api.erdonline.com/mcp。
---

想把正在画的 ER 图交给 Cursor 或 Claude？先[铸造 PAT](https://www.erdonline.com/account/settings?selectKey=personalAccessTokens)，六类客户端都从同一个远程 Streamable HTTP 地址开始：
