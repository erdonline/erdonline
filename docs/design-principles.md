...
- ❌ 字段只能双击编辑、空名静默退出丢类型/PK 改动；仅改类型/PK/非空/自增/隐藏要等 blur 才保存或只能绕表设计；中文名/默认值只能开 EntityModal 或表设计；表头中文名只能开 EntityModal；末行 Tab 退出要再点「+ 添加字段」；Escape 卸编辑后 blur 仍 commit（取消变落盘）；× 删字段无确认；索引/字段表格只能绕左树开表设计（历史问题）
