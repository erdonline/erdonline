...
| `dist/<path>/index.html` | 公开营销路径的独立 SPA 壳（`/catalog`、`/compare`、`/demo`、`/en`、`/en/catalog`、`/en/compare`、`/en/demo`）。爬虫首屏 `<title>` / canonical / hreflang 指向**该路径**，不是首页 Draw-ERD。JSON-LD：`/` = WebApplication；`/catalog` `/en/catalog` = CollectionPage；官方 `/catalog/:id` = ItemPage；其余营销页 = WebPage（不得再套首页 WebApplication + url `/`）。由 `PRERENDER_PAGES` 驱动。`/demo` 真人仍会 hydrate 后跳到 `/s/public-demo`。官方模板另有 `dist/catalog/<id>/index.html`。未知详情 **200 到 `/catalog/` 列表壳**。无 `/en/catalog/:id`。 |
| `_redirects` | **仅**列尚无静态壳的 SPA pathname 的 `200` 反代到 `/`（勿写 `/index.html`，CF 会 308 到根）；**禁止**把已 prerender 的精确路径（`/catalog`、`/compare`、`/en`…）再写回 `/`；**禁止** `/* / 200` 通配；**禁止** `/catalog/*`（splat 会匹配 `/catalog/` 空段，把列表壳盖成首页）。详情：官方 ID 先 identity-200（含尾斜杠），再 `/catalog/:id` `/catalog/:id/` **200 到 `/catalog/`**；`/catalog/_item` **301 到 `/catalog/`**（**不要** 200 到 `/` 或 `_item`） |
...
curl -s https://www.erdonline.com/catalog/demo-authz | grep -E '<title>|rel="canonical"'
