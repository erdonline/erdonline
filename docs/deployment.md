...
| `dist/<path>/index.html` | 公开营销路径的独立 SPA 壳（`/catalog`、`/compare`、`/demo`、`/en`、`/en/catalog`、`/en/compare`、`/en/demo`）。爬虫首屏 `<title>` / canonical / hreflang 指向**该路径**，不是首页 Draw-ERD。由 `PRERENDER_PAGES` 驱动。`/demo` 真人仍会 hydrate 后跳到 `/s/public-demo`。官方模板另有 `dist/catalog/<id>/index.html`（`demo-authz` 等 4 个种子）。未知详情 **200 到 `/catalog/` 列表壳**（不生成 `/catalog/_item`）。无 `/en/catalog/:id`。 |
