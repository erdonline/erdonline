...
直接服务「每周版本保存」的 UI 缺口：分享吊销/管理已 ✅（W2 切片 1）；设计器 chrome 左树去重 + 画布空间回收已 ✅（W2 切片 3）；跨版本 diff 导出已 ✅（W3 切片 1）；version ProList→List + 空态 CTA 已 ✅（W3 切片 2）。剩余 **审批入口理顺**（thin，菜单已有「我的工单/我的审批」）→ 仅排序洁癖；W5 登录品牌壳 / share 顶栏为纯表现层，靠后。
