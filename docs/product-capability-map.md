...
直接服务「每周版本保存」的 UI 缺口：分享吊销/管理已 ✅（W2 切片 1）；设计器 chrome 左树去重 + 画布空间回收已 ✅（W2 切片 3）；设计器内 `calc(100vh)` 清零（树/版本页 flex）已 ✅（W2 切片 4）；跨版本 diff 导出已 ✅（W3 切片 1）；version ProList→List + 空态 CTA 已 ✅（W3 切片 2）；审批/工单入口理顺已 ✅（W3 切片 3）。W5 share 顶栏 ✅（切片 3）；登录品牌壳仍为表现层待办。
