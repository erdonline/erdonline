...
- 部分/过滤索引谓词：PostgreSQL `pg_get_expr(indpred)`、SQL Server `sys.indexes.filter_definition` → `indexs[].filter`（不进 `fields[]`）；MySQL/Oracle 无对等物；导出侧 PG/SQL Server DDL `WHERE` + DBML `note: 'filter: …'`（`@dbml/core` 拒官方 `where:`）
