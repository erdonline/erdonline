...
- MySQL FK：`INFORMATION_SCHEMA.KEY_COLUMN_USAGE` 字典 SQL（失败回退 JDBC）；复合列按 `ORDINAL_POSITION` 拆成多条单字段边
- 后续：PostgreSQL / Oracle / SQL Server 字典 FK；`AssociationEnd.fields[]` 聚合复合键；表清单按需分页
