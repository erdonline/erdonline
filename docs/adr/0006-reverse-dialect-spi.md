...
- 触发器保真：`DialectCapability.supportsTrigger`；MySQL/MariaDB 字典 `INFORMATION_SCHEMA.TRIGGERS` → `entity.triggers[]`（`name` / `timing` / `event` / `statement` / 重建 `ddl`）；失败 warn 跳过；PG / SQL Server / Oracle 另切片
