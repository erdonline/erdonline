...
- 状态：**🚧 进行中**（人工解封 2026-08-04；切片 1–4 ✅ PAT + projects/versions 只读 + MCP 只读骨架）
...
| MCP tool 清单 | **只读骨架已落地**（`mcp/`，`@modelcontextprotocol/sdk`）：`list_projects` / `get_project` / `get_project_schema` / `list_versions` / `get_version` → 上列 REST + PAT。传输：stdio（默认）+ Streamable HTTP（`--http`）。**无写工具**；写版本待切片 5。 |
...
- 明确不做：产品内黑盒「一句话生成 ERD」；自研 LLM；切片 4 仅只读 MCP（无写 tool）
