...
- 状态：**✅ 已接受**（人工解封 2026-08-04；切片 1–5 ✅ + `projects:write` REST/MCP ✅ + Redis 限流 ✅ + **OAuth 切片 A+B** ✅ + **client / PAT 管理 UI** ✅ + **同意页** ✅ + **refresh_token** ✅ + **OIDC 薄 MVP** ✅ + **nonce/at_hash** ✅）
...
| 议题 | 决策 |
|---|---|
| 鉴权模型 | **Personal Access Token（PAT）** + **OAuth**（client_credentials + Authorization Code/PKCE + refresh_token）+ **OIDC 薄 MVP**（discovery / HS256 `id_token` / userinfo）。PAT 前缀 `erd_pat_`；OAuth access token 前缀 `erd_oat_`；refresh 前缀 `erd_ort_`；auth code 前缀 `erd_ac_`。均 `Authorization: Bearer <token>` 调 `/api/v1/**`（**仅** PAT/OAT；refresh **不可**直调 API）。会话 JWT **不**接受于 `/api/v1/**`。 |
...
| OIDC 薄 MVP（✅）+ nonce/at_hash（✅） | `GET /.well-known/openid-configuration`；issuer=`ERD_OIDC_ISSUER` 否则 `ERD_UI_URL`（`martin.ui.url` 首项）。**无**既有 RSA/ES 密钥 → **HS256** `id_token`，密钥 `ERD_OIDC_HMAC`（与 `JWT_SECRET` 分离；prod 无默认 + 拒仓库开发默认串）。`GET /.well-known/jwks.json` 空 keys（HMAC **永不**发布）。`GET /oauth/userinfo`：Bearer OAT + `openid`。`openid` 入 `PatScopes` 白名单（默认铸造仍不含）。**nonce**（可选）：`GET/POST /oauth/authorize?nonce=` → 绑定 `oauth_authorization_code.nonce` → auth code 换票 `id_token` 回显；**refresh 换票不带 nonce**（OIDC Core §12.2）。**at_hash**：auth code / refresh 签发 id_token 时按 access_token 计算（HS256 → SHA-256 左半 + base64url）。**不做**：RSA·真 JWKS / 第三方 IdP 联邦。 |
...
- 代价：新令牌面须吊销/过期/限流运维；文档须强调「明文只见一次」；写 scope 扩大爆破半径须最小权限铸造；OAuth client 吊销须使已发 OAT/ORT 与未消费 code 失效；auth code 流量以授权用户身份访问（异于 client_credentials 的注册人身份）；refresh 复用检测会整族吊销（防盗用，合法并发客户端须串行刷新）；HS256 id_token 依赖共享 `ERD_OIDC_HMAC`（JWKS 空），多租户/公开 RP 宜后置升 RSA
- 明确不做（仍后置）：产品内黑盒「一句话生成 ERD」；自研 LLM；**OIDC 加深**（RSA·真 JWKS / 第三方登录联邦）
