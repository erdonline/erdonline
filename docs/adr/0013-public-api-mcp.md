...
- 状态：**🚧 进行中**（人工解封 2026-08-04；切片 1–5 ✅ + `projects:write` REST/MCP ✅ + Redis 限流 ✅；余 OAuth）
...
| MCP tool 清单 | 只读五件套 + **`create_version`**（`versions:write`）+ **`update_project`** / **`put_project_json`**（`projects:write`）→ REST + PAT。传输：stdio（默认）+ Streamable HTTP（`--http`）。 |
...
- 正面：agent / 脚本有一等鉴权面；北极星可计量「API 产生的版本保存」护栏有挂点；可改项目元数据与工作区 projectJSON（REST + MCP）
- 代价：新令牌面须吊销/过期/限流运维；文档须强调「明文只见一次」；写 scope 扩大爆破半径须最小权限铸造
- 明确不做（本轮仍后置）：产品内黑盒「一句话生成 ERD」；自研 LLM；OAuth 第三方应用
