...
- 状态：**✅ 已接受**（人工解封 2026-08-04；切片 1–5 ✅ + `projects:write` REST/MCP ✅ + Redis 限流 ✅ + **OAuth 切片 A+B** ✅ + **client / PAT 管理 UI** ✅ + **同意页** ✅ + **refresh_token** ✅ + **OIDC 薄 MVP** ✅）
...
| OIDC 薄 MVP（✅） | `GET /.well-known/openid-configuration`；issuer=`ERD_OIDC_ISSUER` 否则 `ERD_UI_URL`（`martin.ui.url` 首项）。**无**既有 RSA/ES 密钥 → **HS256** `id_token`，密钥 `ERD_OIDC_HMAC`（与 `JWT_SECRET` 分离；prod 无默认 + 拒仓库开发默认串）。`GET /.well-known/jwks.json` 空 keys（HMAC **永不**发布）。`GET /oauth/userinfo`：Bearer OAT + `openid`。`openid` 入 `PatScopes` 白名单（默认铸造仍不含）。**不做**：nonce / at_hash / 第三方 IdP 联邦；后置可升 RSA+真 JWKS。 |
...
- 明确不做（仍后置）：产品内黑盒「一句话生成 ERD」；自研 LLM；**OIDC 加深**（nonce / at_hash / RSA·真 JWKS / 第三方登录联邦）
