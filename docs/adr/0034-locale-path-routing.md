...
- **路由门禁**：带 `wrappers: ['@/components/LocaleRoute']` 且有 `routes` 子节点时，子 `path` 必须用相对段（`''`、`:id`、`publish` 等），禁止绝对路径 `/catalog/...`（umi wrapper 会使 React Router `flattenRoutes` 断言失败、整站白屏）。**主门禁**：CI/deploy 的 `yarn check:prod-smoke`（构建后公开 URL 无 init 崩溃）；**辅门禁**：`yarn check:routes` 静态断言。
