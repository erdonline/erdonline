...
| 表面 | 宿主 | 说明 |
|---|---|---|
| 文档站（Docusaurus） | **Cloudflare Pages** 主发（项目 `erdonline-docs`） | 免费档；`main` 经 Actions + Wrangler 部署；**唯一公开 URL** `https://doc.erdonline.com` |
| 文档站回退 | **GitHub Pages** | 同一 `docs-site.yml` 继续部署；无 CF secrets 时仅 GH；**不作为产品入口、不对外链接** |
...
- 与既有 ADR：不推翻 ADR-0003（Docusaurus）；宿主从「仅 GH Pages」扩展为「CF 主 + GH 回退」。公开文档地址只允许 `https://doc.erdonline.com`。
