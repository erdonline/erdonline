...
- **逆向**：MySQL/MariaDB 自 `INFORMATION_SCHEMA.TRIGGERS`、PostgreSQL 自 `information_schema.triggers`（`supportsTrigger`）；名 + 时机/事件 + 体写入上表；`ddl` 为可重建 CREATE（非字节级 `SHOW CREATE TRIGGER` / `pg_get_triggerdef` 克隆）。SQL Server / Oracle 触发器字典另切片。
