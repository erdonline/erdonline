...
| MCP npx | 08-28 | ✅ 30 秒路径改为 `npx -y` GitHub Release tarball，不必 clone+build；npmjs 未发（无登录）。自托管只改 `ERD_API_URL` | `mcpJsonSnippet.ts`、指南、README、PAT 弹层文案 | `mcpJsonSnippet.test.ts`；`npm pack` + `npx -y` 冒烟；未发 XHS；未请求 GSC |
