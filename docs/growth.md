...
| 对照 Agent 格点名 prompt | 08-28 | ✅ `/compare` 格子 + 壳 description：`suggest-erd-version` / 人 diff。**不**发小红书/npm；title 仍 vs draw.io | locales `seo-config.mjs` | `yarn test:seo-static`；`compare.spec.ts` |
