...
- ✅ **版本回滚落盘失败可重试**（2026-08-03）：扫描余假成功——dbsync/Word 导出已收口；`revertVersionData` 仅 save code===200 写 store + 成功 toast；失败不写 store、确认窗不关；`version-revert-failure` 绿；下一刀 → `downloadWordTemplate` JSON/空 blob 假下载或键盘摩擦
