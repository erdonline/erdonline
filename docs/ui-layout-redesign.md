...
- ✅ **DBML 表达式索引**（2026-08-03）：expression↔`indexs[].fields[]` 原样字符串（无 schema 加法）；混列导出 `` `expr` ``；`expression-index.dbml` + unit + `dbml-export` E2E；下一刀 → ADR-0013（人工）/ P4 demo / 逆向函数索引字典
