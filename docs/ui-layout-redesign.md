...
- ✅ **触发器签可编辑已有行**（2026-08-03）：行「编辑」弹层；结构变而 DDL 未动时重建；persist-on-200；失败可重试；`table-triggers`；下一刀 → ADR-0013（人工）/ P4 Railway demo（人工）/ DBML Trigger（等官方块）/ ADR-0011 `fields[]`（仍延期）
