...
- ✅ **左树剪切/粘贴落盘失败可重试**（2026-08-03）：`cutEntity`/`pastEntity`/`cutModule`/`pastModule` `persist:true`；仅 save code===200 写剪贴板与移出/写入 + 成功 toast；失败保留先前状态；复制仅本地剪贴板；`tree-cut-paste-failure` 绿；下一刀 → 改名模型/关系图或拖拽落盘假成功（若仍有）
