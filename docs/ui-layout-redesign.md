...
- ✅ **设计器次屏表密度 / chrome**（2026-08-03）：JExcel 工具栏 ~24 + 表头/行 pad 4×8（压过 datatables）；版本 diff 实体行 token 色；禁 clip；`model-design-ux`「表设计 JExcel 行密度」+ `version.spec` diff 绿；下一刀 → 元数据应用子签 / CodeTab chrome（视 ROI）
