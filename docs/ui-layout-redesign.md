...
- ✅ **CommonTabs 签头键盘闭环**（2026-08-03）：关闭钮「关闭 {表名}」+ 关签焦点归还；`common-tabs-keyboard` 绿；扫余弹层：主 Modal 已闭环，余 `Modal.info` SQL 明细 / `Modal.warning` 导入校验 / databaseConfig Drawer；下一刀 → SQL 明细焦点归还或 Vision 📋
