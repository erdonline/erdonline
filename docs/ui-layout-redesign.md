...
- ✅ **AuthBrandShell 次密距**（2026-08-03）：品牌 32×28/gap14 + 表单 pad32 + 门头 mb16（后更碎至品牌/表单 20×16 + gap12 + 门头 mb12）；登录/注册/失效/404·403 同源；禁弱化品牌字号/Skip·Tab；`smoke`+`share`+`session` densify 绿；下一刀 → ~~LandingChrome / compare 次密距~~✅
...
- ✅ **AuthBrandShell 门头/brand gap 三压**（2026-08-03）：门头 mb16→12 + brand gap14→12；pad 20×16 / 字号 / 色层次不动；`testid=auth-form-header`；`smoke`+`session` 键盘 densify + `share` 绿；下一刀 → AuthBrandShell 表单 Title mt10 / Form 项 antd 默认 mb（视 ROI）
