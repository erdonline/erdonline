...
| `connector/ping|dbReverse*|…` | `JdbcUrlGuard`：仅 `jdbc:mysql\|mariadb\|postgresql\|oracle:(thin\|oci)\|sqlserver`；禁链路本地 / 云 IMDS（`169.254.0.0/16`、`168.63.129.16`、`100.100.100.200`、`fe80::/10`、`fd00:ec2::254`、元数据主机名）；**不**禁 RFC1918/localhost（自托管内网库）；`dataSourceId` 优先（`ConnectorCredentialResolver.apply` → ACL 后覆盖客户端 JDBC 字段） |
