...
Today the only MCP write path is **`create_version`**: the Agent reads the approved contract via `list_tables` / `describe_table`, updates the model, submits a suggestion; you diff in the designer. See [Read ER diagrams from Cursor via MCP]({{DOC:guide/api-and-mcp}}) and [Save a version and view the diff]({{DOC:guide/save-version-and-diff}}).
