ALTER TABLE `${entity.title}` ADD(`${field.name}` ${field.dataType}<#if field.defaultValue?has_content> DEFAULT ${field.defaultValue}</#if><#if field.notNull> NOT NULL</#if>);${separator}
