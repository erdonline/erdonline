...
> **已合入**（勿再投放）：`MiniMap ariaLabel="画布缩略图"`；设计器+分享页；E2E `relation.spec`「MiniMap」。
...
- [x] 设计器 / 分享页 MiniMap 可访问名含中文（如 `getByRole('img', { name: '画布缩略图' })` 或等价）
- [x] 页面上不再出现 `React Flow mini map` 文案（可用 `getByLabel` / `getByText` 断言 count=0）
- [x] 不扩大 `any`；不改 MiniMap 行为（pannable/zoomable 保持）
