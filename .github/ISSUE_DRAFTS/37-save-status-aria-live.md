...
- [ ] `save-status` 节点具备 `aria-live="polite"`（或 `role="status"`，二者其一即可，推荐并存）
- [ ] 可见文案与 `data-testid` / 既有 `aria-label` 模式不变；不改自动保存行为
- [ ] 不扩大 `any`
- [ ] 短 E2E：打开设计器建表触发保存 → `getByTestId('save-status')` 最终为「已保存」，并断言 `aria-live`（或 `role=status`）属性存在
