...
test.describe('版本保存失败可读可重试', () => {
  test('初始化基线：业务码失败不关窗 → 重试成功', async ({ page, request }) => {
    ...
    try {
      ...
      await page.route('**/ncnb/hisProject/save', async (route) => {
        ...
        if (saveHits === 1) {
          await route.fulfill({
            ...
            body: JSON.stringify({ code: 500, msg: '模拟初始化基线拒绝' }),
          });
          return;
        }
        await route.continue();
      ...
      await dialog.getByRole('textbox', { name: '版本描述' }).fill('init fail then retry');
      await dialog.getByRole('button', { name: /确\s*定/ }).click();

      await expectToast(page, '模拟初始化基线拒绝');
      await expect(dialog).toBeVisible();
      await expect(page.getByText('初始化基线成功')).toHaveCount(0);

      await dialog.getByRole('button', { name: /确\s*定/ }).click();
      await expectToast(page, '初始化基线成功');
