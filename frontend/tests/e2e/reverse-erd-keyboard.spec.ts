...
/**
 * 设计器「解析ERD文件」Modal 键盘闭环
 * — 打开首焦上传区；Esc 关；焦点归还「项目菜单」；Tab trap 在 dialog
 * — 不依赖真实 ERD 文件内容；仅走菜单打开的上传 UI
 */
...
test.describe('解析ERD文件弹层键盘', () => {
  test('解析ERD：首焦上传区；Esc 归还；Tab trap', async ({ page }) => {
    test.setTimeout(120_000);
    const projectName = uniqueProjectName('erd-kb');
    try {
      await login(page, e2eAccount());
      await deleteOwnPersonProjects(page);
      await createAndOpenPersonProject(page, projectName, 'erdkb', 'import erd keyboard');
      ...
      const dialog = page.getByRole('dialog', { name: /解析已有ERD文件/ });
      await expect(dialog).toBeVisible({ timeout: 10_000 });
      await expect(
        dialog.getByRole('button', { name: '选择ERD文件' }),
