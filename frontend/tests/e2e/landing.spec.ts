...
test.describe('落地页', () => {
  ...
  test('已登录时主 CTA 显示中文并跳转 /home，不被营销页困住', async ({ page }) => {
    ...
    await expect(heroPrimary).toHaveAccessibleName('中文');
    await expect(page.getByRole('navigation', { name: '落地页导航' }).getByRole('link', { name: '中文' })).toBeVisible();
  });
