...
function writeRedirects(distDir) {
  const lines = [
    ...
    "# Cloudflare Pages: explicit SPA rewrites only; proxy to /app.html.",
    "# / is a static landing page (dist/index.html); all SPA routes use dist/app.html.",
...
function writePrerenderedShells(distDir, indexHtml, siteUrl) {
  ...
  // SPA fallback shell: kept at root as an .html file for CF Pages 200-rewrite.
  fs.rmSync(path.join(distDir, "app"), { recursive: true, force: true });
  fs.rmSync(path.join(distDir, "app.html"), { force: true });
  fs.writeFileSync(path.join(distDir, "app.html"), homeHreflang, "utf8");
