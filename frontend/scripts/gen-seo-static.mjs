...
function writeRedirects(distDir) {
  const lines = [
    ...
    "# Cloudflare Pages: explicit SPA rewrites only; proxy to /app (no-extension shell, not /app/ dir).",
...
function writePrerenderedShells(distDir, indexHtml, siteUrl) {
  ...
  // SPA fallback shell: write as a no-extension file so CF Pages 200-rewrite
  // does not 308-redirect to a directory. Content-Type is set in _headers.
