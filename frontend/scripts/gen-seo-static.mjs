...
function writeRedirects(distDir) {
  const lines = [
    ...
    "# Cloudflare Pages: explicit SPA rewrites only; proxy to /app/ (directory shell).",
    "# / is a static landing page (dist/index.html); all SPA routes use dist/app/index.html.",
...
function writePrerenderedShells(distDir, indexHtml, siteUrl) {
  ...
  // SPA fallback shell: use dist/app/index.html with a trailing-slash 200 rewrite
  // so CF Pages serves it as text/html without exposing an .html suffix.
  fs.rmSync(path.join(distDir, "app"), { recursive: true, force: true });
  fs.rmSync(path.join(distDir, "app.html"), { force: true });
  fs.mkdirSync(path.join(distDir, "app"), { recursive: true });
  fs.writeFileSync(path.join(distDir, "app", "index.html"), homeHreflang, "utf8");
