...
const SqlApproval: React.FC<SqlApprovalProps> = (props) => {
  ...
  return (
    <ModalForm
      ...
      title="发起SQL审批"
      trigger={
        <Button key="approval" type="primary" style={{ display: props.display }}>
          <ConsoleSqlOutlined />
          SQL审批
        </Button>
      }
      ...
      onFinish={async (values: any) => {
        const db = dbs?.find((d: any) => d.name === values.dbInfo);
        if (!db?.properties) {
    ...
    >
      <ProFormSelect
        ...
        request={async () => {
          ...
          return (
            result?.data?.map((m: { id: any; username: any; email: any }) => ({
              value: m.id,
