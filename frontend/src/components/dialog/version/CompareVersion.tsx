...
const CompareVersion: React.FC<CompareVersionProps> = (props) => {
  ...
  console.log('CompareVersion props:', props);
  console.log('1000 CompareVersion state:', {currentVersion, dbVersion, messages, data, versions});
        ...
        <Grid item xs={4} style={{height: '450px', overflowY: "auto"}}>
          {
            messages.length > 0 ?
              ...
              `${data && data.trim() ? '当前脚本为全量脚本' : '当前版本无变化'}`
          }
        </Grid>
        <Grid item xs={8}>
          <CodeEditor
